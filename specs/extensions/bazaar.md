...
**Rule (MUST) — explicit unobservable state.** A service declaring `privacy_class: "fully-private"` emits no anchor by definition. An observation layer MUST represent such services with an explicit `coverage: "unobservable"` state in its public registry view. Observers MUST NOT silently omit fully-private services from the view, and MUST NOT attribute a null or zero count to them. Omission is indistinguishable from missing data; an explicit `unobservable` state is the same declared-not-silent discipline applied to `settlement_count` fallback and observation windows elsewhere in this section.
...
**Rule (MUST) — anchor event discoverability.** So that any neutral indexer (not only the emitter or its auditor) can locate anchors without prior per-emitter configuration, the on-chain anchor MUST be emitted as a typed log event with a stable, registry-declared signature. The event MUST carry, at minimum, the tuple `(service_id, manifest_root, epoch, settlement_count)`, where `service_id` is the emitter's registry identifier. The emitter MUST declare, in its Bazaar registry entry, an `anchor_event` object naming the chain, the emitting address (or address set), and the event signature / topic, so an indexer can subscribe by topic across emitters:
...
"extensions": {
  "bazaar": {
    ...
    "anchor_event": {
      "chain": "base-mainnet",
      "address": "0x...",                       // or "addresses": ["0x...", "0x..."]
      "signature": "ManifestAnchored(bytes32 manifest_root,uint64 epoch,uint64 settlement_count,bytes32 service_id)",
      "topic0": "0x..."                          // keccak256 of signature; stable across emitters
    }
  }
}
```

The signature string and `topic0` are normative for discovery; the parameter encoding is the chain-native log encoding for that signature. Indexers match on `topic0` (identical across all emitters using this event) and read the declared address set per registry entry. Anchor ingest is therefore the same shape of work as existing x402 settlement-event ingest (log subscription per chain, content-hash dedupe, typed table), not a per-emitter integration.

**Rule (MUST) — auditor-availability independence:** The on-chain anchor is the signed JCS attestation carrying `(manifest_root, epoch, settlement_count)` defined above; `manifest_root` is its content-addressing component. Manifest integrity MUST be verifiable independently of the auditor's availability by checking retrieved manifest bytes against the `manifest_root` component of the on-chain anchor tuple. If the auditor service is unreachable, a client that obtains the manifest content from any source (mirror, cache, alternative manifest host) MUST be able to verify that content against `manifest_root` by content-hash equality, without the auditor in the path. The auditor's role is therefore narrowed to "publish the root + serve the manifest content"; it is not a verification bottleneck. An auditor outage MUST NOT stall integrity verification of an already-anchored manifest.

**Implementation guidance (non-normative) — manifest failover order.** The MUST above is the correctness floor; the discovery order is the user-experience layer that decides whether an auditor outage is silent or visible. Clients SHOULD attempt manifest content retrieval in this order, surfacing which tier served the content so an outage is observable rather than masked: (1) the canonical auditor endpoint; (2) a configured mirror list, if declared in the registry entry; (3) content-addressed / decentralized storage (IPFS CID or equivalent) where the CID is derivable from, or committed alongside, the `manifest_root` component of the on-chain anchor. Integrity is enforced at every tier by content-hash equality against the `manifest_root` component of the on-chain anchor, so order affects latency and outage-visibility only, never trust. Clients SHOULD emit a degraded-source signal (telemetry or status field) when content is served from any tier other than the canonical auditor, so silent prolonged auditor outages become detectable. When no retrieval tier produces bytes matching `manifest_root`, the client MUST treat the manifest as unavailable and, if it proceeds with `settlement_count` taken from the on-chain anchor alone, MUST set `manifest_unavailable: true` inside any signed payload it emits. Silent fallback to anchor-only counting without that flag is non-compliant.
